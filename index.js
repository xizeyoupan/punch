...
exports.main_handler = async () => {
    ...
    try {
        const btn = await page.waitForSelector(
            "#app > div > div.css-1dbjc4n.r-105ug2t > div > div > div > div:nth-child(2) > div > div",
            { timeout: 5000 }
        );
        await btn.click();
    } catch (e) { }
    ...
    const btn = await page.waitForSelector(
        "#app > div > div.css-1dbjc4n.r-105ug2t > div > div > div > div:nth-child(2) > div",
        { timeout: 10000 }
    );
    await btn.click();
    await atschoolbtn.click();
    const submitbtn = await page.$(
        "#BT_CLICK > div:nth-child(2) > div:nth-child(2) > div"
    );
